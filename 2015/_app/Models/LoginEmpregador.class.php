...
class LoginEmpregador {
    ...
    public function ExeLogin(array $UserData) {
        $this->Login = (string) strip_tags(trim($UserData['empregador_email']));
        $this->Senha = (string) strip_tags(trim($UserData['empregador_senha']));
        $this->setLogin();
    }
    ...
    public function CheckLogin() {
        if (empty($_SESSION['sess_empregador'])):
            unset($_SESSION['sess_empregador']);
            return false;
        else:
            return true;
        endif;
    ...
    private function getUser() {
        ...
        $read = new Read;
        $read->ExeRead("empregos_empregador", "WHERE empregador_email = :e AND empregador_senha = :s", "e={$this->Login}&s={$this->Senha}");
        if ($read->getResult()):
    ...
    private function Execute() {
        ...
        $_SESSION['sess_empregador'] = $this->Result;

        $this->Error = ["Olá {$this->Result['empregador_nome']}, seja bem vindo(a). Aguarde redirecionamento!", WS_ACCEPT];
        $this->Result = true;
    }
